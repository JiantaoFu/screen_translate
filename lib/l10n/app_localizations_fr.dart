// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for French (`fr`).
class AppLocalizationsFr extends AppLocalizations {
  AppLocalizationsFr([String locale = 'fr']) : super(locale);

  @override
  String get app_title => 'Traduction d\'écran';

  @override
  String get source_language => 'De';

  @override
  String get target_language => 'Vers';

  @override
  String get stop_translation => 'Arrêter la traduction';

  @override
  String get translate_screen => 'Traduire l\'écran';

  @override
  String get source_and_target_cannot_be_the_same =>
      'Les langues source et cible ne peuvent pas être identiques';

  @override
  String get download_model => 'Télécharger';

  @override
  String get cancel => 'Annuler';

  @override
  String get remove => 'Supprimer';

  @override
  String get downloading => 'Téléchargement...';

  @override
  String get overlay_permission_required =>
      'Autorisation de superposition requise';

  @override
  String get overlay_permission_required_content =>
      'L\'appli a besoin de l\'autorisation de s\'afficher par-dessus les autres applis pour montrer les traductions à l\'écran.';

  @override
  String get grant_permission => 'Autoriser';

  @override
  String get language_afrikaans => 'Afrikaans';

  @override
  String get language_albanian => 'Albanais';

  @override
  String get language_arabic => 'Arabe';

  @override
  String get language_belarusian => 'Biélorusse';

  @override
  String get language_bengali => 'Bengali';

  @override
  String get language_bulgarian => 'Bulgare';

  @override
  String get language_catalan => 'Catalan';

  @override
  String get language_chinese => 'Chinois';

  @override
  String get language_croatian => 'Croate';

  @override
  String get language_czech => 'Tchèque';

  @override
  String get language_danish => 'Danois';

  @override
  String get language_dutch => 'Néerlandais';

  @override
  String get language_english => 'Anglais';

  @override
  String get language_esperanto => 'Espéranto';

  @override
  String get language_estonian => 'Estonien';

  @override
  String get language_finnish => 'Finnois';

  @override
  String get language_french => 'Français';

  @override
  String get language_galician => 'Galicien';

  @override
  String get language_georgian => 'Géorgien';

  @override
  String get language_german => 'Allemand';

  @override
  String get language_greek => 'Grec';

  @override
  String get language_gujarati => 'Gujarati';

  @override
  String get language_haitian => 'Haïtien';

  @override
  String get language_hebrew => 'Hébreu';

  @override
  String get language_hindi => 'Hindi';

  @override
  String get language_hungarian => 'Hongrois';

  @override
  String get language_icelandic => 'Islandais';

  @override
  String get language_indonesian => 'Indonésien';

  @override
  String get language_irish => 'Irlandais';

  @override
  String get language_italian => 'Italien';

  @override
  String get language_japanese => 'Japonais';

  @override
  String get language_kannada => 'Kannada';

  @override
  String get language_korean => 'Coréen';

  @override
  String get language_latvian => 'Letton';

  @override
  String get language_lithuanian => 'Lituanien';

  @override
  String get language_macedonian => 'Macédonien';

  @override
  String get language_malay => 'Malais';

  @override
  String get language_maltese => 'Maltais';

  @override
  String get language_marathi => 'Marathi';

  @override
  String get language_norwegian => 'Norvégien';

  @override
  String get language_persian => 'Persan';

  @override
  String get language_polish => 'Polonais';

  @override
  String get language_portuguese => 'Portugais';

  @override
  String get language_romanian => 'Roumain';

  @override
  String get language_russian => 'Russe';

  @override
  String get language_slovak => 'Slovaque';

  @override
  String get language_slovenian => 'Slovène';

  @override
  String get language_spanish => 'Espagnol';

  @override
  String get language_swahili => 'Swahili';

  @override
  String get language_swedish => 'Suédois';

  @override
  String get language_tagalog => 'Tagalog';

  @override
  String get language_tamil => 'Tamoul';

  @override
  String get language_telugu => 'Telugu';

  @override
  String get language_thai => 'Thaï';

  @override
  String get language_turkish => 'Turc';

  @override
  String get language_ukrainian => 'Ukrainien';

  @override
  String get language_urdu => 'Ourdou';

  @override
  String get language_vietnamese => 'Vietnamien';

  @override
  String get language_welsh => 'Gallois';

  @override
  String get enjoying_app => 'Aimez-vous Screen Translate ?';

  @override
  String get review_prompt_message =>
      'Nous aimerions avoir votre avis ! Souhaitez-vous noter l\'application sur Google Play ?';

  @override
  String get rate_now => 'Noter maintenant';

  @override
  String get not_now => 'Pas maintenant';

  @override
  String get cannot_open_store => 'Impossible d\'ouvrir Google Play Store';

  @override
  String get api_key_required => 'Clé API requise';

  @override
  String get go_to_settings => 'Aller aux paramètres';

  @override
  String get api_key_input_error => 'Veuillez entrer une clé API valide';

  @override
  String get api_key_save_error => 'Clé API invalide. Vérifiez et réessayez.';

  @override
  String get translation_mode_on_device => 'Traduction sur l\'appareil';

  @override
  String get translation_mode_on_device_description =>
      'Utilise des modèles de traduction intégrés sur votre appareil. Rapide et fonctionne hors ligne, mais peut avoir un support linguistique et une précision limités.';

  @override
  String get translation_mode_ai => 'Traduction par IA';

  @override
  String get translation_mode_ai_description =>
      'Utilise des modèles d\'IA avancés pour des traductions plus précises et contextuelles. Nécessite une connexion Internet et une clé API.';

  @override
  String get translation_mode_title => 'Mode de traduction';

  @override
  String get close => 'Fermer';

  @override
  String get translation_settings_title => 'Paramètres de traduction';

  @override
  String get translation_quality_section => 'QUALITÉ DE TRADUCTION';

  @override
  String get mode_quick_title => 'Rapide';

  @override
  String get mode_quick_subtitle =>
      'Instantané · Toujours disponible · Aucune configuration requise';

  @override
  String get mode_ai_enhanced_title => 'Amélioré par IA';

  @override
  String get mode_ai_enhanced_subtitle =>
      'Meilleure qualité · Fonctionne hors ligne · Téléchargez un pack de langue';

  @override
  String get mode_cloud_ai_title => 'IA Cloud';

  @override
  String get mode_cloud_ai_subtitle =>
      'Meilleure qualité · Nécessite internet · Clé API requise';

  @override
  String language_packs_header(String size) {
    return 'Packs de langue  ·  ~$size chacun';
  }

  @override
  String pack_downloading_progress(String percent) {
    return 'Téléchargement… $percent%';
  }

  @override
  String get pack_ready_to_use => 'Prêt à l\'emploi';

  @override
  String get pack_failed_tap_retry => 'Échec — appuyez pour réessayer';

  @override
  String get pack_ready_badge => 'Prêt';

  @override
  String get retry => 'Réessayer';

  @override
  String pack_is_ready_snackbar(String name) {
    return '$name est prêt !';
  }

  @override
  String get download_failed_connection =>
      'Échec du téléchargement. Vérifiez votre connexion.';

  @override
  String get remove_language_pack_title => 'Supprimer le pack de langue ?';

  @override
  String remove_ai_pack_confirm(String name) {
    return 'Supprimer le pack IA hors ligne pour $name ?';
  }

  @override
  String remove_quick_pack_confirm(String name) {
    return 'Supprimer le pack de traduction rapide hors ligne pour $name ?';
  }

  @override
  String get failed_to_remove_pack =>
      'Échec de la suppression du pack de langue.';

  @override
  String get connect_ai_account_title => 'Connectez votre compte IA';

  @override
  String get connect_ai_account_prefix => 'Obtenez une clé API gratuite sur ';

  @override
  String get connect_ai_account_suffix => ' et collez-la ci-dessous.';

  @override
  String get api_key_hint_short => 'Collez votre clé API ici';

  @override
  String get save_and_verify => 'Enregistrer et vérifier';

  @override
  String get cloud_ai_connected => 'Connecté ! L\'IA Cloud est prête.';

  @override
  String get advanced_section_label => 'Avancé';

  @override
  String get text_merge_sensitivity_title => 'Sensibilité de fusion du texte';

  @override
  String get text_merge_sensitivity_description =>
      'Contrôle comment les blocs de texte proches sont regroupés. Réduisez si la précision de traduction diminue.';

  @override
  String get merge_precise => 'Précis';

  @override
  String get merge_aggressive => 'Agressif';

  @override
  String get select_language_pair_hint => 'Sélectionnez une paire de langues';

  @override
  String get download_ai_model_title => 'Télécharger le modèle IA local ?';

  @override
  String get download_ai_model_description =>
      'Ce pack fait généralement entre 100 et 500 Mo. Il se télécharge en arrière-plan ; cette paire sera prête à l\'emploi une fois terminé.';

  @override
  String error_prefix(String error) {
    return 'Erreur : $error';
  }

  @override
  String get translate_image_button => 'Traduire l\'image';

  @override
  String get choose_translation_text =>
      'Choisissez comment vous souhaitez traduire le texte';

  @override
  String get download_language_pack_title => 'Télécharger un pack de langue';

  @override
  String get download_language_pack_content =>
      'Pour utiliser le mode Amélioré par IA, téléchargez d\'abord un pack de langue.\n\nAllez dans les paramètres pour en télécharger un.';

  @override
  String get cloud_ai_api_key_required_content =>
      'L\'IA Cloud nécessite une clé API. Configurez-la dans les paramètres.';

  @override
  String get mode_ai_short_label => 'IA';

  @override
  String get mode_cloud_short_label => 'Cloud';

  @override
  String get image_translation_title => 'Traduction d\'image';

  @override
  String get send_feedback => 'Envoyer des commentaires';

  @override
  String get send_feedback_subtitle =>
      'Signaler un bug ou suggérer une fonctionnalité';

  @override
  String get ai_pair_unsupported_title => 'Langue pas encore prise en charge';

  @override
  String get ai_pair_unsupported_content =>
      'Le mode IA n\'a pas encore de modèle pour cette paire de langues. Faites-le-nous savoir — cela nous aide à décider quoi développer ensuite.';

  @override
  String get request_language_pair => 'Demander cette langue';

  @override
  String get language_request_sent =>
      'Merci ! Votre demande a été enregistrée.';

  @override
  String get pack_waiting_for_network => 'En attente du réseau…';

  @override
  String get downloading_language_model_toast =>
      'Téléchargement du modèle de langue (~30 Mo). Veuillez patienter…';
}
